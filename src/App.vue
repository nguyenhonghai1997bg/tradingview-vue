...
import { onMounted, onUnmounted, type HtmlHTMLAttributes } from 'vue';
import {
  createChart,
  CandlestickSeries,
  LineSeries,
...
onMounted(async () => {
  ...
  try {
    ...
    // Function to format values (handle undefined/null and round numbers)
    const formatValue = (value: number | undefined | null, decimals: number = 2): string => {
      if (value === undefined || value === null) return '-';
      return value.toFixed(decimals);
    };

    // Function to update legends with provided data
    const updateLegends = (
